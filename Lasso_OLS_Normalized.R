...
lasso_cv <- cv.glmnet(xx_normal, yy_normal, alpha = 1, type.measure="mse",
...
# total_lambda <- 0
# repeat {
#   cv_model <- cv.glmnet(xx_normal, yy_normal, alpha = 1)
...
n=dim(data_x)[1]
beta_normal = coef(gfit_normal, x=xx_normal, y=yy_normal, s=lambda_cv/n, exact=TRUE)[-1]
library(selectiveInference)
result_normal = fixedLassoInf(xx_normal,yy_normal,beta_normal,lambda_cv,
...
Var
Coef <- result_normal$coef0
#Coef <- Coef_full[-c(1,6,8)]
Coef
LowConfPt <- result_normal$ci[,1]
UpConfPt <- result_normal$ci[,2]
