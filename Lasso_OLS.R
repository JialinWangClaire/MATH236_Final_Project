...
yy<-as.matrix(data_y)
library(glmnet)
lasso.model <- glmnet(xx, yy, alpha = 1, standardize = FALSE)
#windows(width = 15, height = 20)
plot(lasso.model, xvar = "lambda")
legend("bottomright", lwd = 1, col = 1:ncol(xx), legend = colnames(xx), cex = 0.7)
...
beta = coef(gfit, x=xx, y=yy, s=lambda_cv/442, exact=TRUE)[-1]
library(selectiveInference)
