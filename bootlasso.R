...
for (i in 1:times) {
  ...
  lasso_cv <- cv.glmnet(xx, yy, alpha = 1, type.measure="mse",
                        standardize = FALSE, nfolds =10,family="gaussian")

  lasso_cv$lambda.1se
  lambda_cv <- lasso_cv$lambda.1se

  gfit_normal = glmnet(xx,yy,thresh=1E-10,standardize=FALSE)
  n=dim(data_x)[1]
  beta_normal = coef(gfit_normal, x=xx, y=yy, s=lambda_cv/n, exact=TRUE)[-1]

  result_normal = fixedLassoInf(xx,yy,beta_normal,lambda_cv,
